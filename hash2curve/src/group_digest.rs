...
pub trait GroupDigest: MapToCurve {
    /// DST used in the [hash to curve routine](Self::hash_from_bytes).
    const HASH_TO_CURVE_DST: &[u8];
    /// DST used in the [encode to curve routine](Self::encode_from_bytes).
    const ENCODE_TO_CURVE_DST: &[u8];
    ...
    fn hash_from_bytes(msg: &[u8]) -> ProjectivePoint<Self> {
        hash_from_bytes::<Self, Self::ExpandMsg>(&[msg], &[Self::HASH_TO_CURVE_DST])
            .expect("invalid cipher suite")
    }
    ...
    fn encode_from_bytes(msg: &[u8]) -> ProjectivePoint<Self> {
        encode_from_bytes::<Self, Self::ExpandMsg>(&[msg], &[Self::ENCODE_TO_CURVE_DST])
            .expect("invalid cipher suite")
    }
}
