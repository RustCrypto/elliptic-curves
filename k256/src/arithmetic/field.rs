...
mod tests {
    ...
    #[test]
    fn mul_32bit_overflow_check() {
        // A regression test for an error in 32-bit multiplication.

        let x = FieldElement::from_bytes_unchecked(&[
            0xca, 0xec, 0xe7, 0xa0, 0x51, 0xf2, 0x21, 0xd8, 0xb1, 0xc9, 0x8c, 0x7f, 0x67, 0x39,
            0x40, 0x2f, 0xb8, 0xbd, 0x6a, 0x48, 0x78, 0xcb, 0x1a, 0xc4, 0xcc, 0xaa, 0x55, 0x2a,
            0x25, 0x18, 0x21, 0x2b,
        ]);

        let y = FieldElement::from_bytes_unchecked(&[
            0x8b, 0xc0, 0x79, 0x8d, 0x85, 0x54, 0x71, 0x24, 0x7a, 0xd4, 0xa4, 0xa3, 0x28, 0xe7,
            0xc2, 0xf7, 0xc2, 0x45, 0xe4, 0x94, 0x61, 0xb3, 0xa8, 0x0e, 0x27, 0x76, 0xf6, 0xc4,
            0x51, 0x2b, 0x74, 0xda,
        ]);

        let z = &x * &y;

        // Used to fail here (for 32 bit multiplication):
        // After `normalize_weak()`, the top limb had 23 bits, but
        // the condition for that was missing in `get_overflow()`,
        // and a debug assert in `normalize()` fired when the number
        // hadn't been reduced as expected.
        let z_normalized = z.normalize();

        let z_reference = FieldElement::from_bytes_unchecked(&[
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x2d, 0x9b, 0x4d,
            0x34, 0x79, 0x52, 0xcc,
        ]);

        assert_eq!(z_normalized, z_reference);
    }
